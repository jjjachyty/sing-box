...
func (h *Inbound) UpdateUsers(users []adapter.UserEntry) error {
	h.service.UpdateUsers(common.MapIndexed(users, func(index int, it adapter.UserEntry) int {
		return index
	...
	h.users = common.Map(users, func(it adapter.UserEntry) option.VLESSUser {
